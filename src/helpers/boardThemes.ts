...
export const getSquareBackground = (
  theme: BoardTheme,
  isLight: boolean,
): string => {
...
export const createBoardHighlight = (
  light: string,
  dark: string,
): BoardHighlight => ({
...
export const resolveBoardThemeId = (id: string): string =>
  legacyBoardThemeIds[id] ?? id;

export const getBoardThemeById = (id: string): BoardTheme =>
  boardThemes.find((theme) => theme.id === resolveBoardThemeId(id)) ??
  boardThemes[0];
